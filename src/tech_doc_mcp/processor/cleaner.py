...
- 直接输出 Markdown 正文本身；**切勿**用 ``` 代码围栏把整篇输出整体包起来（代码围栏只用于文中真正的代码片段）
...
# 匹配"整篇被散文语言围栏包裹"的开头（LLM 输出常见 artifact）
_OUTER_PROSE_FENCE = re.compile(r"^```(?:markdown|md|mdx|text|txt)\s*\n", re.I)


def unwrap_outer_prose_fence(text: str) -> str:
    """若整篇被 ```markdown/text 等散文围栏包裹，则解包（去首尾伪围栏、留正文）。

    处理 LLM 把整页输出包进 ```markdown 的 artifact：首行是散文语言围栏、末尾是 ```，
    去掉这一对外层围栏，保留中间正文（含其中合法的内部代码块）。仅当解包后内部
    ``` 数量为偶数（结构平衡）才动手，否则保守不动。
    """
    t = text.strip()
    m = _OUTER_PROSE_FENCE.match(t)
    if not m or not t.endswith("```"):
        return text
    inner = t[m.end():-3]
    if inner.count("```") % 2 != 0:  # 内部围栏不平衡 → 结构异常，保守不动
        return text
    return inner.strip()
    ...
    async def process(md_file: Path) -> None:
        async with sem:
            ...
            try:
                ...
                result = unwrap_outer_prose_fence(result)  # 兜底:解包整篇散文围栏
