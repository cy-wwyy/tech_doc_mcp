...
**状态:基础服务可用。** 核心链路 爬取 → 清洗 → 索引 → serve → 搜索 全部跑通并验证:
- MCP 服务实测可启动，监听 `127.0.0.1:8000/mcp`，4 个 tool 挂载正常
- 搜索融合完成重构（剔除 BM25 → presence + 饱和乘性 boost-only），已用 `eval/` 评测体系用数据敲定并落地，5 个源可搜
- 接入方式见 `README.md` 阶段二（客户端配置 Claude Code MCP）

**一期到此结束。** 下一步（Web 管理界面 D4 等）暂缓，不阻塞使用。剩余待办见下方「待处理」。
